...
<template>
  <div class="container-box" ref="containerBox">
    <div class="container-header">
      <h2>{{$t('layoutNav.medicalRecordManage')}}</h2>
  ...
  export default {
    ...
    methods: {
      getList () {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/medicalRecord/list.jhtml',
          success: function(res){
            ...
            if(_vm.langRouterKey !== 'medicalRecord') {
              _vm.data = _data;
            } else {
      ...
      add () {
        this.modalTit = this.$t('medicalRecord.modalTit1');
        this.$refs['formValidate'].resetFields();
        this.seen = false
        this.modalEdit = true
      },
      edit (data) {
        this.modalTit = this.$t('medicalRecord.modalTit2');
        this.$refs['formValidate'].resetFields();
      ...
      remove (data) {
        ...
        _vm.$Modal.confirm({
          ...
          content: `<p>${_vm.$t('medicalRecord.msg1')}</p>
          <p>${_vm.$t('medicalRecord.msg2')}</p>
          <p>${_vm.$t('labelPublic.mc')}：${data.nameLang}</p>`,
          onOk: () => {
            _vm.$http.post({
              url: 'guard-webManager/medicalRecord/del.jhtml',
              data: {id: data.id},
              success: function(res){
                if(res.data.code === 0){
                  _vm.getList()
      ...
      tobuild (row) {
        ...
        let _query = {id: row.id, name: row.name}
      ...
      ok () {
        ...
        _vm.$refs['formValidate'].validate((valid) => {
          if (valid) {
            ...
            let data = {};
            if(_vm.seen) {
              url = 'guard-webManager/medicalRecord/edit.jhtml'
              data = _vm.formValidate
            } else {
              url = 'guard-webManager/medicalRecord/add.jhtml'
              data = {
