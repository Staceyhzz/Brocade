            ...
            <Row type="flex" justify="start">
              <Col v-for='(item, index) in formList' :key='index' :span='item.col' class="form-item">
                ...
                <FormItem :label="item.name" :required='item.required === 1' v-if='item.type === 2'>
                  <Input :placeholder="item.placeholder" :disabled='item.status === 0' :autosize="{minRows: 2,maxRows: 2}" type="textarea" v-model.trim="item.result" :maxlength="item.maxlength"></Input>
                  <p v-show='item.repeat === 0 && item.repeatMsg' style="color: #FF6F76" @on-blur="validRepeatInput(item.repeat, true, index)">数据已存在，请重新填写！</p>
                </FormItem>
    ...
    methods: {
      ...
      getCusList() {
        ...
        _vm.$http.get({
          ...
          data: {
            customerId: _vm.formShow.customerId,
            hospitalId: localStorage.getItem('hospId')
          },
          success: function (res) {
            if(res.data.code === 0) {
        ...
        if(resultList.length) {
          resultList.map(item => {
            ...
            if(indexR >= 0) {
              ...
              let obj = {
                customerId: this.formShow.customerId,
                userId: localStorage.getItem('userId'),
                hospitalId: localStorage.getItem('hospId'),
