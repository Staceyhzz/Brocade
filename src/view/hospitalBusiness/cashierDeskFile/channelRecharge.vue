      ...
      <Form :model='formShow' :label-width="100" label-position="right">
        <Row type="flex" justify="start">
          <Col :xs='24' :sm='24' :md="12" :lg="12">
						<Form-item label="渠道商">
							<span>{{formShow.distributionUserName}} ({{formShow.distributionuserID}})</span>
						</Form-item>
					</Col>
					<Col :xs='24' :sm='24' :md="12" :lg="12">
  ...
  export default {
    ...
    methods: {
      getName() {
        let _vm = this;
        _vm.$http.get({
          url: 'guard-webManager/distributionSet/getVirtualcurrencyName  ',
          success: function (res) {
            if (res.data.code === 0) {
              _vm.col[1].title = '获得的' + res.data.content;
            }
          },
          error: function (res) {
            console.log(res);
      ...
      getList(id) {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/cashierDesk/cashierDistributionRechargeOrder.jhtml',
          data: {
            id: id
          },
          success: function (res) {
            ...
            const _data = [{
              price: obj.amount,
              total: obj.virtualCurrencyBalance
            }];
      ...
      save() {
        ...
        _vm.$refs['formAdd'].validate((valid) => {
          if (valid) {
            ...
            _vm.saveLoad = true
            _vm.$http.post({
              url: 'guard-webManager/cashierDesk/orderDistributionRechargeEdit.jhtml',
              data: _vm.formAdd,
              success: function (res) {
                if (res.data.code === 0) {
                  console.log(res)
