  ...
  export default {
    ...
    methods: {
      getList() {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/bed/list.jhtml',
      ...
      add() {
        this.modalTit = this.$t('bed.modalTit1');
        this.$refs['formValidate'].resetFields();
        this.seen = false
        this.modalEdit = true
      },
      edit(data) {
        this.modalTit = this.$t('bed.modalTit2');
        this.$refs['formValidate'].resetFields();
      ...
      remove(data) {
        ...
        _vm.$Modal.confirm({
          ...
          content: `<p>${_vm.$t('bed.msg1')}</p>
          <p>${_vm.$t('bed.msg2')}</p>
          <p>${_vm.$t('labelPublic.mc')}：${data.nameLang}</p>`,
          onOk: () => {
            _vm.$http.post({
              url: 'guard-webManager/bed/del.jhtml',
              data: {
                id: data.id
              },
              success: function (res) {
      ...
      ok() {
        ...
        _vm.$refs['formValidate'].validate((valid) => {
          if (valid) {
            let url = '';
            if (_vm.seen) {
              url = 'guard-webManager/bed/update.jhtml'
            } else {
              url = 'guard-webManager/bed/add.jhtml'
            }
            _vm.$http.post({
              url: url,
