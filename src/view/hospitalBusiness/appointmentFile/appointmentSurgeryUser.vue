...
<template>
  <div class="container-box" ref='wrapper'>
    <div class="container-header">
      <h2>{{$t('appointmentToday.menu4')}}——{{userName}}</h2>
  ...
  export default {
    ...
    methods: {
      getList () {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/appointmentDesk/surgeryUser.jhtml',
          data: _vm.formSearch,
          success: function(res){
            ...
            const _days = moment(_vm.$route.query.date).daysInMonth()
            const _workList = res.data.content.ShiftList || []
            const _nameBox = res.data.content.StartTime.split('-')
            let start = res.data.content.surgeryStart
            const end = res.data.content.surgeryEnd
            ...
            if(_vm.langRouterKey !== 'appointmentSurgeryUser') {
