...
<template>
  <div class="container-box" ref='wrapper'>
    <div class="container-header">
      <h2>{{$t('appointmentToday.menu2')}}——{{userName}}</h2>
  ...
  export default {
    ...
    methods: {
      getList () {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/appointmentDesk/consultUser.jhtml',
          data: _vm.formSearch,
          success: function(res){
            ...
            let start = res.data.content.consultStart
            const end = res.data.content.consultEnd
            ...
            if(_vm.langRouterKey !== 'appointmentConsultUser') {
            ...
            render: (h, params) => {
              if(item.workType === 0) {} else {
                if(params.row.dataKey) {
                  let _dataList = params.row.dataList.filter(cus => cus.appointmentdate.split(' ')[0] === item.name)
                  return h('div', {
                    class: 'table-box',
                    style: {
                      height: params.row.realHeight + 'px'
