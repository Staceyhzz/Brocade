  ...
  export default {
    ...
    data () {
      return {
        ...
        bodyHeight: null,
        opacityKey: 100,
        menuRouteUrl: '/appointmentConsult',
        formSearch: {
    ...
    methods: {
      ...
      getList () {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/appointmentDesk/consultIndex.jhtml',
          data: _vm.formSearch,
          success: function(res){
            _vm.hospList = res.data.content.hospital || []
            _vm.countData = res.data.content.consultList.map(item => {
              item.status = item.status === 0 ? 0 : 1
              return item
            }) || []
            const _docList = res.data.content.userList || []
            const _docStatusList = res.data.content.shiftList || []
            let start = moment().format('YYYY-MM-DD') + ' ' + res.data.content.consultStart.split(' ')[1]
            const end = moment().format('YYYY-MM-DD') + ' ' + res.data.content.consultEnd.split(' ')[1]
            const interval = res.data.content.consultInterval
            ...
            const _data = res.data.content.consultList.map(item => {
              item.status = item.status === 0 ? 0 : 1
              return item
            }) || []
            if(_vm.langRouterKey !== 'appointmentConsult') {
              ...
              _vm.requestList(_data, ['content']).then(res => {
        ...
        if(_vm.timer) {
          ...
          setTimeout(() => {
            _vm.$nextTick(() => {
              ...
              console.log(_vm.$refs.treat.$el.clientHeight)
      ...
      setData (data, interval) {
        ...
        this.timeBox.map((item, index) => {
          if(index < this.timeBox.length - 1) {
            ...
            const _consultList = data.filter(cus => {
            ...
            if(_consultList.length) {
              _obj.dataKey = true
              _consultList.map(consult => {
                const _heightStart = moment(consult.appointmentdate.split(' ')[0] + ' ' + consult.appointmentstarttime)
                const _heightEnd = moment(consult.appointmentdate.split(' ')[0] + ' ' + consult.appointmentendtime)
                const _topStart = moment(consult.appointmentdate.split(' ')[0] + ' ' + item + ':00')
                const _heightDiff = _heightEnd.diff(_heightStart, 'm')
                const _topDiff = _heightStart.diff(_topStart, 'm')
                consult.height = _heightDiff / (interval * 1) * (_obj.realHeight + 1) + 'px'
                consult.top = _topDiff / (interval * 1) * _obj.realHeight + 'px'
                consult.time = item + '-' + this.timeBox[index + 1]
                consult.color = consult.status === -1 ? 'rgba(24,24,24,0.15)' : consult.status === 1 ? 'rgba(65,114,246,0.25)' : 'rgba(255,111,118,0.25)'
                consult.borderColor = consult.status === -1 ? '#3d3d3d' : consult.status === 1 ? '#4172F6' : '#FF6F76'
                // consult.startText = consult.status === -1 ? '已取消' : consult.status === 1 ? '已上门' : '未上门'
                consult.openKey = false
                // 统一格式数据
                consult.customerID = consult.customerid
                consult.appointmentStartTime = consult.appointmentstarttime
                consult.appointmentEndTime = consult.appointmentendtime
                consult.remark = consult.content
                _obj.dataList.push(consult)
              })
            }
            this.data.push(_obj)
          }
      ...
      toAdd(id, date, start, end, type) {
        let breadData = [
          ...
          {
            url: '/appointmentConsultAdd',
            text: 'appointmentConsultAdd.tit'
          }
        ]
        this.$store.dispatch('setBreadData', breadData)
        this.$router.push({
