  ...
  export default {
    ...
    methods: {
      ...
      getList () {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/treatScheduler/index.jhtml',
          data: _vm.formSearch,
          success: function(res){
            ...
            let start = moment().format('YYYY-MM-DD') + ' ' + res.data.content.appointmentStart.split(' ')[1]
            const end = moment().format('YYYY-MM-DD') + ' ' + res.data.content.appointmentEnd.split(' ')[1]
            ...
            if(_vm.langRouterKey !== 'appointmentTreat') {
      ...
      setDocWork (interval) {
        const realHeight = (interval * 1) < 30 ? 60 : interval * 2
        const _start = moment().format('YYYY-MM-DD') + ' ' + this.timeBox[0] + ':00' 
        const _end = moment().format('YYYY-MM-DD') + ' ' + this.timeBox[this.timeBox.length - 1] + ':00' 
        let _first = this.data[0]
        let _finally = this.data[this.data.length - 1]
        const firstUnWork = this.docTimeList.filter(item => {
          const _firstTime = moment().format('YYYY-MM-DD') + ' ' + item.starttime
          if((moment(_firstTime).isAfter(_start) && moment(_end).isAfter(_firstTime))) {
            const _heightDiff = moment(_firstTime).diff(moment(_start), 'm')
            item.height = _heightDiff / (interval * 1) * realHeight + 'px'
            return item
          }
        })
        const finallyUnWork = this.docTimeList.filter(item => {
          const _finallyTime = moment().format('YYYY-MM-DD') + ' ' + item.endtime
          if((moment(_finallyTime).isAfter(_start) && moment(_end).isAfter(_finallyTime))) {
            const _topDiff = moment(_finallyTime).diff(moment(_end), 'm')
            item.top = _topDiff / (interval * 1) * realHeight + 'px'
            return item
          }
        })
        if(firstUnWork.length) {
          _first.firstKey = true
          _first.firstList = firstUnWork
          this.data.splice(0, 1, _first)
        }
        if(finallyUnWork.length) {
          _finally.finallyKey = true
          _finally.finallyList = finallyUnWork
          this.data.splice(this.data.length - 1, 1, _finally)
        }
        console.log(this.data)
      },
      //添加
