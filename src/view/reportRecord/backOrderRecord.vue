/**
 * 项目退单数据
 *【原表】由：集团退项目单明细、医院退项目单明细、用户退项目单明细、集团退预收款单明细、医院退预收款单明细、用户退预收款单明细合并。
 */
  ...
  <div class="container-box">
    ...
    <div class="container-body">
      ...
      <Form :label-width="90" :model="formSearch" ref='formSearch' label-position="right">
        <Row>
          <Col :sm='12' :md="8" :lg="6">
            <Form-item label="退单时间" prop='startTime'>
          ...
          <Col :sm='12' :md="8" :lg="6">
            <Form-item label="订单类型" prop='orderType'>
              <Select v-model="formSearch.orderType" clearable>
                <Option :value="1">退项目单</Option>
                <Option :value="2">退预收款单</Option>
              </Select>
            </Form-item>
          </Col>
          <Col :sm='12' :md="8" :lg="6">
          ...
          <Col :sm='12' :md="8" :lg="6">
            <Form-item :label-width="90" label="应收总额" prop='compare' style='display:inline-block'>
          ...
          <Col :sm='12' :md="8" :lg="6">
            <Form-item :label-width="90" label="积分扣减" prop='compare1' style='display:inline-block'>
              <Select v-model="formSearch.compare" clearable style='width:50px'>
                <Option v-for="item in compareList" :label="item" :value="item" :key="item">{{ item }}</Option>
              </Select>
            </Form-item>
            <Form-item :label-width='1' prop='point' style="display:inline-block">
                <cmptInputNumber :max="1000000" :min="0" v-model="formSearch.point"></cmptInputNumber>
            </Form-item>
          </Col>
          <Col :sm='12' :md="8" :lg="6">
            <Form-item :label-width="90" label="现金支付" prop='compare2' style='display:inline-block'>
              <Select v-model="formSearch.compare" clearable style='width:50px'>
                <Option v-for="item in compareList" :label="item" :value="item" :key="item">{{ item }}</Option>
              </Select>
            </Form-item>
            <Form-item :label-width='1' prop='backAmount' style="display:inline-block">
                <cmptInputNumber :max="1000000" :min="0" v-model="formSearch.backAmount"></cmptInputNumber>
            </Form-item>
          </Col>
          <Col :sm='12' :md="8" :lg="6">
            <Form-item :label-width="90" label="退回余额" prop='compare3' style='display:inline-block'>
              <Select v-model="formSearch.compare" clearable style='width:50px'>
                <Option v-for="item in compareList" :label="item" :value="item" :key="item">{{ item }}</Option>
              </Select>
            </Form-item>
            <Form-item :label-width='1' prop='backAmount1' style="display:inline-block">
                <cmptInputNumber :max="1000000" :min="0" v-model="formSearch.backAmount1"></cmptInputNumber>
            </Form-item>
          </Col>
          <Col :sm='12' :md="8" :lg="6">
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            width: 60,
            render: (h, params) => {
              const text = params.row.orderType === 1
                ? '退项目单'
                : '退预收款单';
              return h('span', text);
            }
          ...
          },
          {
            ...
            key: 'backAmount1',
            render: (h, params) => {
              if(params.row.backAmount1 || params.row.backAmount1 === 0) {
                return h('span', params.row.backAmount1.toFixed(2))
              }
            }
          },
          {
        ...
        _vm.$http.get({
          ...
          success: function(res){
            ...
            if(res.data.code === 0) {
              ...
              const _col = _vm.tableCount(data, ['amount', 'point', 'backAmount1', 'backAmount']);
              _col.map(item => {
