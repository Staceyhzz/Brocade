    ...
    <div class="container-box">     
        ...
        <div class="container-body">
            <Form ref="formEdit" :model='formEdit' :label-width="70" label-position="right" style='margin-top:10px;' :rules="ruleForm">
                ...
                <Form-item label="领用日期" prop='createDate'>
                    <!-- <Date-picker :transfer='true' type="date" placeholder="选择日期" style="width: 300px" :value='formEdit.createDate' :clearable='false' @on-change='setTime' :options="options"></Date-picker> -->
                    <span>{{formEdit.createDate}}</span>
                </Form-item>
                <Form-item label="领用部门" prop='deptId' required>
                    <Select v-model="formEdit.deptId" style="width:300px" @on-change='getUserList'>
                        <Option v-for="item in deptList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                    </Select>
                </Form-item>
                <Form-item label="领用人" prop='useUserId' required>
                    <Select v-model="formEdit.useUserId" style="width:300px">
                        <Option v-for="item in useUserList" :value="item.id" :key="item.id">{{ item.name }}</Option>
                    </Select>
                </Form-item>
                <Form-item label="备注" prop="remark">
                    <Input v-model.trim="formEdit.remark" type="textarea" :autosize="{minRows: 2,maxRows: 5}" placeholder="请输入备注"></Input>
                </Form-item>
    ...
    export default {
        ...
        methods: {
            ...
            getUserList (id) {
                let _vm = this
                _vm.$http.get({
                    url: 'guard-webManager/select/deptUserList.jhtml',
                    data: {
                        deptId: _vm.formEdit.deptId
                    },
                    success: function(res){
                        if(res.data.code == 0 ){
                            console.log(res)
                            _vm.useUserList = res.data.content
                        } else {
            ...
            save () {
                ...
                _vm.$refs['formEdit'].validate((valid) => { 
                    if (valid) {
                        ...
                        _vm.$http.post({
                            url: 'guard-webManager/use/add.jhtml',
                            data: _vm.formEdit,
                            success: function(res){
                                if(res.data.code == 0) {
                                    _vm.$router.push('/use')
