...
<template>
  <div class="container-box">
    <div class="container-header">
      <h2>{{$t('userAuthCustomSet.msg5')}}</h2>
    </div>
  ...
  export default {
    ...
    methods: {
      getList () {
        ...
        _vm.$http.getPage({
          ...
          success: function(res){
            ...
            let _all = res.data.content.allCuMenuList
            let _now = res.data.content.cuMenuList
            _all.map(item => {
              _now.map(item1 => {
                if(item.id === item1.id) {
                  item['_checked'] = true
      ...
      save () {
        ...
        _url = ''
        if(_vm.key) {
          _url = 'guard-webManager/userAuthCustom/updateCuMenu.jhtml'
        } else {
          _url = 'guard-webManager/role/updateCuMenuEdit.jhtml'
        }
        _vm.saveLoad = true
        _vm.$http.post({
          url: _url,
