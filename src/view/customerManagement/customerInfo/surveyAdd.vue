...
<template>
  <div class="container-box userPower">
    <div class="container-header">
      <h2>{{$t('cusNavList.customerSurvey')}}</h2>
    </div>
...
<script>
  export default {
    ...
    created() {
      this.formValidate.surveyId = this.$route.query.id;
    ...
    methods: {
      ...
      getId() {
        ...
        _vm.$http.getPage({
          url: 'guard-webManager/customerRecord/surveyAdd.jhtml',
      ...
      save() {
        ...
        _vm.$refs['formValidate'].validate((valid) => {
          if (valid) {
            ...
            _vm.$http.post({
              url: 'guard-webManager/customerRecord/addSurvey.jhtml',
              data: _vm.formValidate,
              success: function (res) {
                if (res.data.code === 0) {
                  const breadData = [
