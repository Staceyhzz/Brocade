...
<template>
  <div class="order-view order-update">
    <div class="container-box">
      <div class="container-header">
        <h2>{{$t('indexOrder.tit7')}}</h2>
      </div>
      ...
      getId(id) {
        ...
        _vm.$http.getPage({
          ...
          success: function (res) {
            ...
            const order = res.data.content.Order || {};
						_vm.formEdit.remark = order.remark || '';
            _vm.doctorList = eval(res.data.content.Arr) || [];
            _vm.formEdit.editData = order.detailList.map(item => {
              return {
                assignDoctor: item.doctorUserID
                  ? 1
                  : 0,
                id: item.chargeID,
                ...
                realPrice: item.price / item.num,
                sell: item.price
                  ? parseFloat((item.finalPrice / item.price * 100).toFixed(2))
                  : 100,
                setid: item.setID
                  ? item.setID
                  : 0,
