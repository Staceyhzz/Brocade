    ...
    <div class="container-body">
      ...
      <Form :label-width="80" inline :model="formSearch" ref='formSearch'>
        ...
        <Form-item label="提交用户" prop='createUserId'>
          <cmptHospChooseUser _placeHolder="选择用户" :_style="{width:'162px'}" :_userName="createUserName"
...
<script>
  export default {
      ...
      return {
        ...
        col: [{
          ...
          },
          {
            ...
            key: 'virtualCurrency',
            render: (h, params) => {
              let text = params.row.virtualCurrency.toFixed(2)
              return h('span', text)
            }
          },
          {
    ...
    computed: {
      ...
      exportPrefixURL() {
        return this.$store.getters.getExportPrefixURL + '/guard-webManager/excelExport/hospitalSettlementDetail.jhtml'
      }
    },
    created() {
    ...
    methods: {
      getList(pageNumber, form) {
        ...
        _vm.$http.get({
          url: 'guard-webManager/report/hospitalSettlementDetail.jhtml',
          data: form,
        ...
        _vm.$http.get({
          ...
          success: function (res) {
            if(res.data.code === 0) {
              _vm.col.find(item => item.key === 'virtualCurrency').title = res.data.content ? res.data.content : '虚拟币';
            }
          },
          error: function (res) {
            console.log(res);
